// BonitoWidgets client module.
//
// The workspace layout engine — tabs, splits, floating windows, and all the
// drag/dock/float/resize gestures — lives here as a proper ES6 module rather
// than an inline `js"""` block. Two reasons:
//   1. It loads once as a static asset (hashed, cached), instead of being
//      re-serialized into every render.
//   2. It stays interactive under Bonito's `export_static`: inline JSCode
//      callbacks + interpolated-observable subscriptions don't fully wire up in
//      a static export, but a plain module called with the (serialized)
//      observables + DOM nodes as arguments does.
//
// The Julia side renders the DOM regions and owns the layout STATE (the
// `layout`/`meta`/`closed` observables); this module owns the LOGIC and only
// syncs state back via `observable.notify(...)`. Entry point: `mountWorkspace`.
...
// Mount the layout engine on an already-rendered `.bw-ws` root.
//
// opts:
//   chrome, parking, floatlayer  — the three DOM regions (rendered by Julia)
//   layout, meta, closed         — Bonito observables (the synced state)
//   minFraction                  — smallest split fraction (number)
//   hideSingleTab                — drop the tab bar for a lone panel (bool)
export function mountWorkspace(opts) {
    ...
    // A full render() parks every panel through the display:none pool and then
    // re-parents it into the new layout — both of which reset the scrollTop of
    // any scrollable descendant (the chat message list, an editor) to 0. Snapshot
    // every scrolled descendant's offset BEFORE parking and restore it AFTER the
    // panels are re-placed, so docking / closing / floating a sibling panel no
    // longer jumps the chat to the top. (Tab switches use setActiveLocal and skip
    // render() entirely, so they never needed this.)
